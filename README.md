# Codex, Gemini & Auggie plugins for Claude Code

Use Codex, Gemini, and Auggie from inside Claude Code for code reviews or to delegate tasks.

This project provides three Claude Code plugins:
...
- **Auggie plugin** — integrates Augment CLI (Auggie) for code review and adversarial review
...
- `/auggie:review` for a read-only Auggie code review
- `/auggie:adversarial-review` for a steerable adversarial review powered by Auggie
- `/auggie:setup` to check Auggie CLI availability and authentication
...
- **For Auggie commands:** Augment account. [Learn more](https://docs.augmentcode.com).

## Install

### Option A: Install from this fork via marketplace (includes Codex, Gemini, and Auggie)
...
```bash
/plugin install codex@trunkszi-ai-review
/plugin install gemini@trunkszi-ai-review
/plugin install auggie@trunkszi-ai-review
```
...
cd codex-plugin-cc
claude --plugin-dir ./plugins/codex --plugin-dir ./plugins/gemini --plugin-dir ./plugins/auggie
```
...
claude --plugin-dir ./plugins/auggie   # Auggie only
```
...
> [!NOTE]
> The official marketplace only includes the Codex plugin. The Gemini and Auggie plugins are available through this fork (Option A or B).
...
If Auggie CLI is not installed, follow the [official installation guide](https://docs.augmentcode.com).

If Auggie CLI is installed but not authenticated:
...
- the Auggie slash commands (`/auggie:*`)
...
Yes. Load all plugins with:

```bash
claude --plugin-dir ./plugins/codex --plugin-dir ./plugins/gemini --plugin-dir ./plugins/auggie
```

The commands use different prefixes (`/codex:*`, `/gemini:*`, and `/auggie:*`) so they do not conflict.

---

# Auggie plugin for Claude Code

Use Augment CLI (Auggie) from inside Claude Code for code reviews.

This plugin is for Claude Code users who want to use Auggie as an additional review tool. Auggie supports multiple model backends including Claude, Gemini, and GPT models.
...
Runs a standard Auggie code review on your current work. The review runs in ask mode (`-a`) ensuring read-only execution.
...
```bash
/auggie:review
/auggie:review --base main
/auggie:review --background
/auggie:review --model opus4.6 --wait
```

This command is read-only and will not perform any changes.

### `/auggie:adversarial-review`

Runs a **steerable** adversarial review that questions the chosen implementation and design.

It can be used to pressure-test assumptions, tradeoffs, failure modes, and whether a different approach would have been safer or simpler.

It uses the same review target selection as `/auggie:review`, including `--base <ref>` for branch review.
It also supports `--wait`, `--background`, and `--model <model>`. Unlike `/auggie:review`, it can take extra focus text after the flags.
...
```bash
/auggie:adversarial-review
/auggie:adversarial-review --model gpt5.4 --wait look for race conditions
/auggie:adversarial-review --base main challenge whether this was the right design
/auggie:adversarial-review --background
```
...
Checks whether Auggie CLI is installed and authenticated.

```bash
/auggie:setup
```

## Typical Auggie Flows
...
```bash
/auggie:adversarial-review --wait
```

### Use A Specific Model

```bash
/auggie:adversarial-review --model opus4.6 --wait
/auggie:adversarial-review --model gpt5.4 --wait
/auggie:adversarial-review --model gemini-3.1-pro-preview --wait
```
...
The Auggie plugin invokes the [Augment CLI](https://docs.augmentcode.com) in one-shot ask mode (`auggie -p -a --quiet -i <instruction>`). Each review spawns a single `auggie` process that runs to completion.

Review context (git diff, file contents) is piped via stdin with randomized boundary tags to prevent prompt injection. The instruction prompt is passed via the `-i` flag.

The plugin reuses shared infrastructure from the Codex plugin for git context collection, argument parsing, and workspace resolution.

## Auggie FAQ

### What models does Auggie support?

Auggie supports multiple model backends. Run `auggie model list` to see all available models. As of version 0.22.0, supported models include:

- Claude: haiku4.5, sonnet4.5, sonnet4.6, opus4.5, opus4.6
- Gemini: gemini-3.1-pro-preview
- GPT: gpt5, gpt5.1, gpt5.2, gpt5.4

### Do I need a separate account?

Yes. Auggie requires an Augment account. Run `auggie login` to authenticate.

### Can I use Auggie alongside Codex and Gemini?

Yes. All three plugins use different command prefixes and do not conflict. Install them all via the marketplace or load them from local directories.
